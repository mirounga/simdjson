// Shared std::simd string-parsing finders (no include guard of its own; emitted
// once per implementation namespace, like the other generic/*.h kernel bodies).
//
// `backslash_and_quote` / `escaping` are the copy-and-find primitives the stage-2
// string unescaper (src/generic/stage2/stringparsing.h) and the JSON string builder
// use: each step copies a 64-byte block to the destination and reports the quote /
// backslash / control positions as bitmasks. Every SIMD backend rides the full
// 64-byte `block` here (same width as stage 1); only `fallback` keeps a scalar
// finder. Relies on `block`, `dp`, `load_block`, `to_bitmask` (from the kernel) and
// `trailing_zeroes` (bitmanipulation) already being in scope -- the per-ISA wrapper
// includes <isa>/simd.h and <isa>/bitmanipulation.h before this header.
...
// Processes a full 64-byte block per string-parsing step: the kernel `block`
// directly, no simd8 wrapper. bs/quote/escape masks are 64-bit.
