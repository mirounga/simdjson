#ifndef SIMDJSON_SRC_SVE_CPP
#define SIMDJSON_SRC_SVE_CPP

#ifndef SIMDJSON_CONDITIONAL_INCLUDE
#include <base.h>
#endif // SIMDJSON_CONDITIONAL_INCLUDE

#include <simdjson/sve.h>
#include <simdjson/sve/implementation.h>

#include <simdjson/sve/begin.h>
...
simdjson_warn_unused error_code implementation::minify(const uint8_t *buf, size_t len, uint8_t *dst, size_t &dst_len) const noexcept {
  return sve::stage1::json_minifier::minify<64>(buf, len, dst, dst_len);
}
...
simdjson_flatten simdjson_warn_unused error_code dom_parser_implementation::stage1(const uint8_t *_buf, size_t _len, stage1_mode streaming) noexcept {
  this->buf = _buf;
  this->len = _len;
  return sve::stage1::json_structural_indexer::index<64>(buf, len, *this, streaming);
}

simdjson_warn_unused bool implementation::validate_utf8(const char *buf, size_t len) const noexcept {
  return sve::stage1::generic_validate_utf8(buf,len);
}
...
simdjson_warn_unused uint8_t *dom_parser_implementation::parse_string(const uint8_t *src, uint8_t *dst, bool allow_replacement) const noexcept {
  return sve::stringparsing::parse_string(src, dst, allow_replacement);
}

simdjson_warn_unused uint8_t *dom_parser_implementation::parse_wobbly_string(const uint8_t *src, uint8_t *dst) const noexcept {
  return sve::stringparsing::parse_wobbly_string(src, dst);
}
...
#include <simdjson/sve/end.h>

#endif // SIMDJSON_SRC_SVE_CPP
