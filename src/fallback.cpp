...
// and ASCII white-space ('\r','\n','\t',' '). Expressed via the shared std::simd
// kernel's lookup_16 (which uses the portable scalar gap fill on fallback) over the
// whole 64-byte block at once -- byte-identical to the SIMD backends, just scalarized.
// The whitespace/op tables encode pshufb low-nibble lookups; the [ ]/{ } pair is folded
// with | 0x20 (see the table comments below).
...
simdjson_warn_unused error_code implementation::minify(const uint8_t *buf, size_t len, uint8_t *dst, size_t &dst_len) const noexcept {
  return fallback::stage1::json_minifier::minify<128>(buf, len, dst, dst_len);
}

simdjson_warn_unused error_code dom_parser_implementation::stage1(const uint8_t *_buf, size_t _len, stage1_mode streaming) noexcept {
  this->buf = _buf;
  this->len = _len;
  return fallback::stage1::json_structural_indexer::index<128>(_buf, _len, *this, streaming);
}

simdjson_warn_unused bool implementation::validate_utf8(const char *buf, size_t len) const noexcept {
  return fallback::stage1::generic_validate_utf8(buf,len);
}
